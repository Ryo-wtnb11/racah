...
Revision `1804afc6e2b349dd388dbb5474e321f7185ba66a`; `cgc-gen`; crate-internal workload, so consumer revision is N/A. Measured with `rustc 1.96.0 (ac68faa2)`, macOS/aarch64, release test binary, and the test-only wrapper around `System`.
...
| phase | cold ns | warm ns | reset-before-each ns |
| --- | ---: | ---: | ---: |
| SU(2) | 451625 | 1000 | 271250 |
| SU(3) | 24348958 | 3833 | 19195916 |
| SU(4) | 22156708 | 2500 | 29926375 |
...
`charged bytes` are cache-entry charge only. The System-wrapper `allocator_live_bytes` is Rust allocation traffic visible through `GlobalAlloc`; it includes non-cache allocations and excludes C/library allocations. RSS is `null` on this macOS run because no dependency-free portable collector was introduced. One raw run is recorded, not a timing distribution: rerun the command in fresh processes before quoting medians/MAD.
...
3. Aggregate scaling without shared LRU: **recommend** preserving independent tiers; both sequential orders have identical per-tier accumulation and no eviction evidence for cross-tier competition.
4. Public shared CGC/F results: **defer**; the clone trace proves a cost exists, but changing ownership is a public API decision.
