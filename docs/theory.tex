...
\large A self-contained note on irreps, Clebsch--Gordan coefficients,\\
recoupling, gauge, and global forms}
...
group: irrep labels and dimensions, dualities and Frobenius--Schur indicators,
...
\begin{enumerate}[label=(\roman*),leftmargin=2.2em]
  \item \textbf{What is the object?} A definition, with every symbol defined
    before use (Sections~\ref{sec:compact}--\ref{sec:global}).
...
document's bibliography verbatim; $[14]$--$[20]$ are added here for results
this note states that \refsmd{} did not previously need to source.
...
%======================================================================
\section{Global forms: which group, not which algebra}
\label{sec:global}
%======================================================================

Everything above is stated for a group, but a root system names only an
algebra. $\so(N)$ is the algebra of both $Spin(N)$ and
...
tensor/spinor split of Section~\ref{sec:bcd}. $Spin(2r+1)$ admits all
...
\end{equation}
for every $r$ --- this is the tensor/spinor split of Section~\ref{sec:bcd},
...
the order of the Dynkin label tuple in the API.
